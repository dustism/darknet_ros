 ...
 * Author: Yichen Hu
 * Institute: New York University, Agile Robotics and Perception lab
 */
...
YoloDetector::YoloDetector(ros::NodeHandle nh)
    : nodeHandle_(nh),
      ...
      rosBoxCounter_(0)
{
  ROS_INFO("[YoloDetector] Node started.");
...
bool YoloDetector::readParameters()
{
  ...
  if (XOpenDisplay(NULL)) {
    // Do nothing!
    ROS_INFO("[YoloDetector] Xserver is running.");
  } else {
    ROS_INFO("[YoloDetector] Xserver is not running.");
    viewImage_ = false;
  }
  // Set vector sizes.
...
void YoloDetector::init()
{
  ...
  // Path to data folder.
  nodeHandle_.param("data_path", dataPath, std::string("/default"));
  ...
  setupNetwork(cfg, weights, data, thresh, detectionNames, numClasses_,
                0, 0, 1, 0.5, 0, 0, 0, 0);
  yoloThread_ = std::thread(&YoloDetector::yolo, this);
  ...
  imageSubscriber_ = imageTransport_.subscribe(cameraTopicName, cameraQueueSize,
                                               &YoloDetector::cameraCallback, this);
  ...
  checkForObjectsActionServer_->registerGoalCallback(
      boost::bind(&YoloDetector::checkForObjectsActionGoalCB, this));
  checkForObjectsActionServer_->registerPreemptCallback(
      boost::bind(&YoloDetector::checkForObjectsActionPreemptCB, this));
  checkForObjectsActionServer_->start();
}

void YoloDetector::cameraCallback(const sensor_msgs::ImageConstPtr& msg)
{
  ROS_DEBUG("[YoloDetector] USB image received.");
...
void YoloDetector::checkForObjectsActionGoalCB()
{
  ROS_DEBUG("[YoloDetector] Start check for objects action.");
...
void YoloDetector::checkForObjectsActionPreemptCB()
{
  ROS_DEBUG("[YoloDetector] Preempt check for objects action.");
  checkForObjectsActionServer_->setPreempted();
}

bool YoloDetector::isCheckingForObjects() const
{
...
// double YoloDetector::getWallTime()
// {
//   struct timeval time;
...
int YoloDetector::sizeNetwork(network *net)
{
  int i;
  int count = 0;
  for(i = 0; i < net->n; ++i){
...
void YoloDetector::rememberNetwork(network *net)
{
  int i;
  int count = 0;
  for(i = 0; i < net->n; ++i){
...
detection *YoloDetector::avgPredictions(network *net, int *nboxes)
{
  int i, j;
  int count = 0;
...
void *YoloDetector::detectInThread()
{
  running_ = 1;
  float nms = .4;
...
void *YoloDetector::fetchInThread()
{
  {
...
/*void *YoloDetector::displayInThread(void *ptr)
{
...
void *YoloDetector::displayLoop(void *ptr)
{
  while (1) {
    displayInThread(0);
  }
...
void *YoloDetector::detectLoop(void *ptr)
{
  while (1) {
    detectInThread();
  }
...
void YoloDetector::setupNetwork(char *cfgfile, char *weightfile, char *datafile, float thresh,
...
void YoloDetector::yolo()
{
  ...
  while (!demoDone_) {
    ...
    fetch_thread = std::thread(&YoloDetector::fetchInThread, this);
    detect_thread = std::thread(&YoloDetector::detectInThread, this);
    if (!demoPrefix_) {
...
IplImageWithHeader_ YoloDetector::getIplImageWithHeader()
{
...
bool YoloDetector::getImageStatus(void)
{
  boost::shared_lock<boost::shared_mutex> lock(mutexImageStatus_);
  return imageStatus_;
}

bool YoloDetector::isNodeRunning(void)
{
...
void *YoloDetector::publishInThread()
{
  ...
  if (isCheckingForObjects()) {
    ROS_DEBUG("[YoloDetector] check for objects in image.");
